...
static unordered_map<std::string, int> divisionMap = {
    {"1/2", 960}, {"1/4", 480}, {"1/8", 240}, {"1/16", 120}, {"1/32", 60},
};

void DrawGrid(ImDrawList *draw_list, ImVec2 origin, ImVec2 size,
              int clip_length, std::string divisionString, int y_range) {
...
void DrawGridEvents(ImDrawList *draw_list, std::vector<GridEvent> &events) {
  ...
  for (const auto &event : events) {
    draw_list->AddRectFilled(event.position,
                             ImVec2(event.position.x + event.size.x,
                                    event.position.y + event.size.y),
                             event.color);

    // Check for click within the event box
    if (ImGui::IsMouseHoveringRect(event.position,
                                   ImVec2(event.position.x + event.size.x,
                                          event.position.y + event.size.y)) &&
        ImGui::IsMouseClicked(0)) {
      // Handle event click (here we are just setting isClicked to true for
      // demonstration)
      const_cast<GridEvent &>(event).isClicked =
          true; // Note: This cast is necessary to modify the const event in the
                // vector
    }
  }
}
void DrawMidiEvents(ImDrawList *draw_list, ImVec2 &origin, ImVec2 &windowSize,
                    shared_ptr<MidiClip> clip) {
  ImGuiIO &io = ImGui::GetIO();
  auto length = clip->getLength();
  auto minMaxNotes = clip->getNoteRange();
  auto noterange = minMaxNotes.second - minMaxNotes.first;
  float y_spacing;
  (noterange == 0) ? y_spacing = windowSize.y
                   : y_spacing = windowSize.y / (float)noterange;

  for (const auto &[noteNumber, events] : clip->data) {
    for (auto &event : events) {
      auto xStart =
          origin.x + 1 + ((float)event.start / (float)length) * windowSize.x;
      auto yStart = (origin.y + windowSize.y - 2) -
                    y_spacing * (float)(noteNumber - minMaxNotes.first);
...
void DrawPlayhead(ImDrawList *draw_list, ImVec2 &origin, ImVec2 &windowSize,
                  shared_ptr<MidiClip> clip) {
  ImU32 color = IM_COL32(225, 225, 225, 127);
  auto clipPosition =
      windowSize.x * ((float)clip->getCounter() / (float)clip->getLength()) +
      origin.x;
  draw_list->AddLine(ImVec2(clipPosition, origin.y),
                     ImVec2(clipPosition, origin.y + windowSize.y), color, 5.f);
}
